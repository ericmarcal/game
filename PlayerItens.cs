...
public class PlayerItens : MonoBehaviour
{
    ...
    [Header("Configurações de Drop")] // << NOVO HEADER
    [SerializeField] private float itemDropForce = 2f; // << NOVO
    [System.Serializable]
    ...
    public void ConsumeFirstAvailableHotbarItem()
    {
        for (int i = 0; i < hotbarSlots.Count; i++)
        {
            InventorySlot slot = hotbarSlots[i];
            if (slot != null && slot.item != null && slot.item.isConsumable)
            {
                ConsumeItemInSlot(ContainerType.Hotbar, i);
                return;
            }
        }
    }
    ...
    public void DropItemToWorld(InventorySlot itemSlot)
    {
        if (itemSlot == null || itemSlot.item == null || itemSlot.item.itemPrefab == null) return;
        Player player = FindObjectOfType<Player>();
        Vector3 dropPosition = player != null ? player.transform.position : transform.position;
        Vector2 dropDirection = player != null ? player.lastMoveDirection : Vector2.down;
        GameObject itemGO = Instantiate(itemSlot.item.itemPrefab, dropPosition, Quaternion.identity);
        WorldItem worldItem = itemGO.GetComponent<WorldItem>();
        if (worldItem != null)
        {
            worldItem.itemData = itemSlot.item;
            worldItem.quantity = itemSlot.quantity;
            worldItem.isMagnetic = false;
            worldItem.pickupDelay = 1f;

            // << CHAMADA ATUALIZADA E CORRIGIDA >>
            worldItem.SetupSpawnedItemParameters(dropPosition, dropDirection, itemDropForce);
        }
    }
