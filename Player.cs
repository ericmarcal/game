...
public class Player : MonoBehaviour, IDamageable
{
    ...
    [Tooltip("O tempo, em segundos, que a animação de morte leva para tocar.")]
    ...
    private void Start() { initialSpeed = speed; if (spriteRenderer != null) originalSpriteColor = spriteRenderer.color; currentHealth = maxHealth; currentStamina = maxStamina; UseHotbarItem(currentHotbarIndex); }
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Tab)) { if (InventoryManager.instance != null) { InventoryManager.instance.ToggleInventory(!InventoryManager.instance.IsInventoryOpen()); } }

        if (isPaused || IsBusy())
        {
            if (_isAttacking && canChainToNextCombo && Input.GetMouseButtonDown(0)) HandleAttackInput();
            if (_isAttacking && canChainToNextCombo && (Time.time - lastChainInputTime > comboChainWindowDuration)) FinishAttackSequence();
            else if (_isAttacking && !canChainToNextCombo && attackComboCount > 0 && (Time.time - lastChainInputTime > comboChainWindowDuration + 0.5f)) FinishAttackSequence();
            return;
        }
    ...
    private void FixedUpdate()
    {
        ...
        if (IsBusy())
        {
            if (_isRolling)
            {
                rig.velocity = rollDirection.normalized * rollSpeed;
            }
            else
            {
                rig.velocity = Vector2.zero;
    ...
    private void HandleActionInputs()
    {
        ...
        {
            Vector3Int facingTile = grid.WorldToCell(playerActionPoint.position);
            if (FarmingManager.instance.CanHarvest(facingTile))
            {
                FarmingManager.instance.Harvest(facingTile);
            }
        }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (currentSlot?.item?.itemType == ItemType.Semente)
            {
                Vector3Int facingTile = grid.WorldToCell(playerActionPoint.position);
                FarmingManager.instance.Plant(facingTile, currentSlot.item.cropData);
            }
            else if (currentTool == ToolType.Sword)
            {
                HandleAttackInput();
            }
            else if (currentTool != ToolType.None)
            {
                HandleToolAction();
            }
        }
